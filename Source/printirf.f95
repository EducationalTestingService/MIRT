...
subroutine printirf(factorname,itemname,numcatobs,unitirf,resid,&
...
if(resid)then
	write(unit=unitirf,fmt=fmt1,iostat=io) "Item_name",",",&
		"Item_freq",",","Cat_no",",",(trim(adjustl(factorname(row))),',',row=1,size(factorname)),"Cond_irf",",",&
		"Uncond_irf",",",&
		"Res_irf",",","Std_err_res_irf",",",&
		"Adjusted_residual"
else
	write(unit=unitirf,fmt=fmt1,iostat=io) "Item_name",",",&
		"Item_freq",",","Cat_no",",","Cond_irf",",",&
		"Uncond_irf"
end if
...
do item=1,size(itemname)
	do cat=1,numcatobs(item)
		...
		do point=1,size(factorname)
			...
			write(unit=unitirf,fmt=fmt1,iostat=io)trim(adjustl(itemname(item))),",",trim(adjustl(buff1)),&
				(",",trim(adjustl(buff(row1))),row1=1,row)
